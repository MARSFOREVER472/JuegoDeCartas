...
- **_Si estás interesado en aprender a programar Javascript, con este proyecto vas a tener una gran oportunidad de aprender diversos temas relacionados con dichas herramientas de JavaScript que son tremendamente importantes, como por ejemplo, la manipulación del ```DOM```, la creación de elementos y/o números aleatorios, a cómo acceder a un ```index``` de cualquier ```Array```, acerca de cómo crear una función en Javascript, a cómo usar las plantillas ```string```'s + interpolación en Javascript, aprende a usar los métodos matemáticos (```floor``` & ```random```(Ya que estas variables forman parte de la función ```Math``` en Javascript)), entre otros!_**
